...
.btn-header{display:inline-flex;align-items:center;gap:6px;padding:5px 14px;font-family:var(--mono);font-size:11px;font-weight:bold;letter-spacing:1px;text-transform:uppercase;border:1px solid;cursor:pointer;transition:all .2s;text-decoration:none}
.btn-refresh-header{background:rgba(0,200,255,.08);border-color:var(--accent);color:var(--accent)}
.btn-refresh-header:hover{background:rgba(0,200,255,.2)}
.btn-refresh-header.loading{opacity:.5;pointer-events:none}
.btn-collect{background:rgba(0,229,122,.08);border-color:var(--green);color:var(--green)}
.btn-collect:hover{background:rgba(0,229,122,.2)}
.btn-config{background:rgba(255,170,0,.08);border-color:var(--amber);color:var(--amber)}
.btn-config:hover{background:rgba(255,170,0,.2)}
...
.modal-btn{flex:1;padding:8px;font-family:var(--mono);font-size:11px;font-weight:bold;letter-spacing:1px;text-transform:uppercase;border:1px solid;cursor:pointer;transition:all .2s}
.modal-btn-save{background:rgba(255,170,0,.1);border-color:var(--amber);color:var(--amber)}
.modal-btn-save:hover{background:rgba(255,170,0,.2)}
.modal-btn-cancel{background:rgba(0,0,0,.2);border-color:var(--border2);color:var(--text2)}
...
<div class="modal-overlay" id="modal-config">
  <div class="modal">
    ...
    <div class="modal-field">
      ...
      <div class="modal-info">Gere em: github.com → Settings → Developer settings → Personal access tokens → Tokens (classic) → Scope: workflow</div>
    </div>
    <div class="modal-field">
    ...
    <div class="modal-field">
      <label class="modal-label">Workflow (nome do arquivo .yml)</label>
    ...
    <div class="modal-field">
      ...
      <div class="modal-info">Informativo apenas — o email e configurado via secret EMAIL_TO no GitHub Actions</div>
    </div>
...
function salvarConfig(){
  ...
  st.textContent='Configuracoes salvas!';
  st.className='modal-status ok';
  st.style.display='block';
  setTimeout(()=>fecharConfig(),1500);
}

// ── Disparar coleta via GitHub API ────────────────────────────
async function dispararColeta(){
  const c=carregarConfig();
  if(!c.token){
    abrirConfig();
    const st=document.getElementById('modal-status');
    st.textContent='Configure o Token GitHub para disparar a coleta!';
    st.className='modal-status err';
    st.style.display='block';
    return;
  }
  const btn=document.getElementById('btn-coletar');
  btn.innerHTML='<span class="spin">&#9654;</span> Coletando...';
  btn.style.opacity='0.6';
  btn.style.pointerEvents='none';
  try{
    const resp=await fetch(
      `https://api.github.com/repos/${c.repo}/actions/workflows/${c.workflow}/dispatches`,
      {
        method:'POST',
        headers:{
    ...
    if(resp.status===204){
      btn.innerHTML='&#10003; Coleta iniciada!';
      btn.style.borderColor='var(--green)';
      btn.style.color='var(--green)';
      setTimeout(()=>{
        btn.innerHTML='&#9654; Coletar Agora';
        btn.style.opacity='1';
        btn.style.pointerEvents='auto';
        btn.style.borderColor='var(--green)';
        btn.style.color='var(--green)';
      },4000);
      // Aguarda 90s e atualiza os dados
      setTimeout(()=>forcarAtualizacao(),90000);
    } else {
      const err=await resp.json().catch(()=>({}));
      throw new Error(err.message||'Status '+resp.status);
    }
  }catch(e){
...
async function forcarAtualizacao(aba){
  ...
  if(btn){btn.classList.remove('loading');btn.innerHTML=aba?'&#8634; Atualizar':'&#8634; Atualizar';}
}
