...
<title>GNA GERACAO - DESSEM/ONS</title>
...
.btn-header{display:inline-flex;align-items:center;gap:6px;padding:5px 14px;font-family:var(--mono);font-size:11px;font-weight:bold;letter-spacing:1px;text-transform:uppercase;border:1px solid;cursor:pointer;transition:all .2s;text-decoration:none;background:transparent}
.btn-refresh-header{border-color:var(--accent);color:var(--accent)}
.btn-refresh-header:hover{background:rgba(0,200,255,.15)}
.btn-collect{border-color:var(--green);color:var(--green)}
.btn-collect:hover{background:rgba(0,229,122,.15)}
.btn-config{border-color:var(--amber);color:var(--amber)}
.btn-config:hover{background:rgba(255,170,0,.15)}
...
.modal{background:var(--bg2);border:1px solid var(--border);border-top:3px solid var(--amber);padding:24px;width:100%;max-width:480px;font-family:var(--mono)}
.modal-title{font-family:var(--head);font-size:16px;font-weight:700;letter-spacing:2px;text-transform:uppercase;color:var(--amber);margin-bottom:16px}
.modal-field{margin-bottom:14px}
.modal-label{font-size:10px;letter-spacing:2px;text-transform:uppercase;color:var(--text3);margin-bottom:6px;display:block}
.modal-input{width:100%;background:var(--bg);border:1px solid var(--border2);color:var(--text);font-family:var(--mono);font-size:12px;padding:8px 12px;outline:none}
.modal-input:focus{border-color:var(--amber)}
.modal-info{font-size:10px;color:var(--text3);margin-top:4px;line-height:1.5}
.modal-btns{display:flex;gap:10px;margin-top:20px}
.modal-btn{flex:1;padding:8px;font-family:var(--mono);font-size:11px;font-weight:bold;letter-spacing:1px;text-transform:uppercase;border:1px solid;cursor:pointer;transition:all .2s;background:transparent}
.modal-btn-save{border-color:var(--amber);color:var(--amber)}
.modal-btn-save:hover{background:rgba(255,170,0,.15)}
.modal-btn-cancel{border-color:var(--border2);color:var(--text2)}
.modal-btn-cancel:hover{border-color:var(--text);color:var(--text)}
.modal-status{font-size:11px;margin-top:10px;padding:8px;border:1px solid;display:none}
.modal-status.ok{color:var(--green);border-color:var(--green);background:rgba(0,229,122,.08)}
.modal-status.err{color:var(--red);border-color:var(--red);background:rgba(255,60,90,.08)}
/* Meta */
...
/* Graficos */
.chart-area{padding:20px;display:grid;grid-template-columns:1fr 1fr;gap:16px}
@media(max-width:900px){.chart-area{grid-template-columns:1fr}}
.chart-box{background:var(--bg3);border:1px solid var(--border);padding:16px;position:relative}
.chart-box-title{font-family:var(--head);font-size:11px;font-weight:700;letter-spacing:2px;text-transform:uppercase;color:var(--text2);margin-bottom:12px}
...
<div class="modal-overlay" id="modal-config">
  <div class="modal">
    ...
    <div class="modal-field">
      <label class="modal-label">Token GitHub (para disparar coleta)</label>
      <input class="modal-input" type="password" id="cfg-token" placeholder="ghp_xxxxxxxxxxxxxxxxxxxx">
      <div class="modal-info">github.com > Settings > Developer settings > Personal access tokens > Classic > scope: workflow</div>
    </div>
    <div class="modal-field">
      <label class="modal-label">Repositorio GitHub</label>
      <input class="modal-input" type="text" id="cfg-repo" value="LeandroSouza1234561/gna_dessem4">
    </div>
    <div class="modal-field">
      <label class="modal-label">Workflow (.yml)</label>
      <input class="modal-input" type="text" id="cfg-workflow" value="coletar.yml">
    </div>
    <div class="modal-field">
      <label class="modal-label">Email para notificacoes</label>
      <input class="modal-input" type="email" id="cfg-email" placeholder="leandro.souza@gna.com.br">
      <div class="modal-info">Configurado via secret EMAIL_TO no GitHub Actions</div>
    </div>
    <div class="modal-status" id="modal-status"></div>
    <div class="modal-btns">
      <button class="modal-btn modal-btn-save" onclick="salvarConfig()">&#10003; Salvar</button>
      <button class="modal-btn modal-btn-cancel" onclick="fecharConfig()">&#10005; Fechar</button>
    </div>
  </div>
</div>
...
<header>
  ...
  <div class="header-right">
    <div class="header-btns">
      <button class="btn-header btn-collect" id="btn-coletar" onclick="dispararColeta()">&#9654; Coletar Agora</button>
      <button class="btn-header btn-refresh-header" id="btn-refresh-header" onclick="forcarAtualizacao()">&#8634; Atualizar</button>
      <button class="btn-header btn-config" onclick="abrirConfig()">&#9881; Config</button>
...
<div class="tabs">
  ...
  <button class="tab-btn tger" onclick="trocarTab('ger',this)">Tabela Geral</button>
...
<div class="panel" id="tab-gna1">
  <div class="panel-header">
    ...
    <span class="panel-title">GNA I - Graficos</span>
    <div class="panel-info" id="info-chart-gna1">-</div>
    <button class="btn-refresh" onclick="forcarAtualizacao()">&#8634; Atualizar</button>
  </div>
  <div class="chart-area">
    <div class="chart-box">
      <div class="chart-box-title">ClinGter - GNA I (MW)</div>
      <canvas id="chart-gna1-clingter"></canvas>
    </div>
    <div class="chart-box">
      <div class="chart-box-title">CMO - GNA I (R$/MWh)</div>
      <canvas id="chart-gna1-cmo"></canvas>
    </div>
    <div class="chart-box">
      <div class="chart-box-title">CMB - GNA I (R$/MWh)</div>
      <canvas id="chart-gna1-cmb"></canvas>
    </div>
    <div class="chart-box">
      <div class="chart-box-title">GTER - GNA I (MW)</div>
      <canvas id="chart-gna1-gter"></canvas>
    </div>
  </div>
</div>
...
<div class="panel" id="tab-gna2" style="display:none">
  <div class="panel-header">
    ...
    <span class="panel-title">GNA II - Graficos</span>
    <div class="panel-info" id="info-chart-gna2">-</div>
    <button class="btn-refresh" onclick="forcarAtualizacao()">&#8634; Atualizar</button>
  </div>
  <div class="chart-area">
    <div class="chart-box">
      <div class="chart-box-title">ClinGter - GNA II (MW)</div>
      <canvas id="chart-gna2-clingter"></canvas>
    </div>
    <div class="chart-box">
      <div class="chart-box-title">CMO - GNA II (R$/MWh)</div>
      <canvas id="chart-gna2-cmo"></canvas>
    </div>
    <div class="chart-box">
      <div class="chart-box-title">CMB - GNA II (R$/MWh)</div>
      <canvas id="chart-gna2-cmb"></canvas>
    </div>
    <div class="chart-box">
      <div class="chart-box-title">GTER - GNA II (MW)</div>
      <canvas id="chart-gna2-gter"></canvas>
    </div>
  </div>
</div>

<!-- ABA TABELA GERAL -->
<div class="panel" id="tab-ger" style="display:none">
  <div class="panel-header">
    <div class="panel-accent ca"></div>
    <span class="panel-title">GNA GERACAO - Tabela</span>
...
<script>
let dadosGlobal=null,historicoGlobal=[],colunas=[],colunasTerm=[],sortState={col:null,dir:'asc'},tabAtual='gna1',periodoAtual='hoje';
...
function carregarConfig(){
  return{
    token:    localStorage.getItem('gna_token')||'',
    repo:     localStorage.getItem('gna_repo')||'LeandroSouza1234561/gna_dessem4',
    workflow: localStorage.getItem('gna_workflow')||'coletar.yml',
    email:    localStorage.getItem('gna_email')||'leandro.souza@gna.com.br',
  };
}
function abrirConfig(){
  ...
  document.getElementById('cfg-workflow').value=c.workflow;
  document.getElementById('cfg-email').value=c.email;
  document.getElementById('modal-status').style.display='none';
  document.getElementById('modal-config').classList.add('open');
}
function fecharConfig(){document.getElementById('modal-config').classList.remove('open');}
function salvarConfig(){
  localStorage.setItem('gna_token',    document.getElementById('cfg-token').value.trim());
  localStorage.setItem('gna_repo',     document.getElementById('cfg-repo').value.trim());
  localStorage.setItem('gna_workflow', document.getElementById('cfg-workflow').value.trim());
  localStorage.setItem('gna_email',    document.getElementById('cfg-email').value.trim());
  const st=document.getElementById('modal-status');
  st.textContent='Configuracoes salvas!';st.className='modal-status ok';st.style.display='block';
  setTimeout(()=>fecharConfig(),1500);
}

// ── Coleta via GitHub API ───────────────────────────────────
async function dispararColeta(){
  const c=carregarConfig();
  if(!c.token){abrirConfig();const st=document.getElementById('modal-status');st.textContent='Configure o Token GitHub!';st.className='modal-status err';st.style.display='block';return;}
  const btn=document.getElementById('btn-coletar');
  btn.innerHTML='<span class="spin">&#9654;</span> Coletando...';btn.style.opacity='0.6';btn.style.pointerEvents='none';
  try{
    const resp=await fetch(`https://api.github.com/repos/${c.repo}/actions/workflows/${c.workflow}/dispatches`,
      {method:'POST',headers:{'Authorization':'Bearer '+c.token,'Accept':'application/vnd.github+json','Content-Type':'application/json'},body:JSON.stringify({ref:'main'})});
    if(resp.status===204){
      btn.innerHTML='&#10003; Iniciado!';btn.style.borderColor='var(--green)';btn.style.color='var(--green)';
      setTimeout(()=>{btn.innerHTML='&#9654; Coletar Agora';btn.style.opacity='1';btn.style.pointerEvents='auto';btn.style.borderColor='var(--green)';btn.style.color='var(--green)';},4000);
      setTimeout(()=>forcarAtualizacao(),90000);
    } else {throw new Error('Status '+resp.status);}
  }catch(e){
    btn.innerHTML='&#10005; Erro';btn.style.borderColor='var(--red)';btn.style.color='var(--red)';
    setTimeout(()=>{btn.innerHTML='&#9654; Coletar Agora';btn.style.opacity='1';btn.style.pointerEvents='auto';btn.style.borderColor='var(--green)';btn.style.color='var(--green)';},3000);
  }
}

// ── Graficos ───────────────────────────────────────────────
function criarOuAtualizarGrafico(id, labels, data, cor, tipo='line'){
  ...
  charts[id]=new Chart(ctx,{
    ...
    data:{
      ...
      datasets:[{
        label:'',data,
        borderColor:cor,
        backgroundColor: tipo==='bar'? cor+'33' : cor+'22',
    ...
    options:{
      ...
      plugins:{legend:{display:false},tooltip:{
        backgroundColor:'#0d1520',borderColor:cor,borderWidth:1,
        titleColor:cor,bodyColor:'#c8dff5',
        callbacks:{label:ctx=>`${ctx.parsed.y?.toLocaleString('pt-BR',{minimumFractionDigits:2,maximumFractionDigits:2})}`}
      }},
      scales:{
        x:{ticks:{color:'#3d5a78',font:{size:9},maxRotation:45},grid:{color:'rgba(30,58,95,.3)'},border:{color:'#1e3a5f'}},
        y:{ticks:{color:'#6a8faf',font:{size:10},callback:v=>v.toLocaleString('pt-BR',{maximumFractionDigits:0})},grid:{color:'rgba(30,58,95,.3)'},border:{color:'#1e3a5f'}}
      }
    }
  });
}

function renderGraficos(planta, registros){
  const sufixo = planta==='GNA I'?'gna1':'gna2';
  const cor     = planta==='GNA I'?'#00c8ff':'#ffaa00';
  const regs    = registros.filter(r=>r.planta_id===planta);
  if(!regs.length) return;

  const labels    = regs.map((_,i)=>`P${i+1}`);
  const clingter  = regs.map(r=>r['ClinGter']??r['CLINGTER']??null);
  const cmo       = regs.map(r=>r['CMO']??null);
  const cmb       = regs.map(r=>r['CMB']??null);
  const gter      = regs.map(r=>r['GTER']??null);

  criarOuAtualizarGrafico(`chart-${sufixo}-clingter`, labels, clingter, cor, 'line');
  criarOuAtualizarGrafico(`chart-${sufixo}-cmo`,      labels, cmo,      '#cc88ff', 'line');
  criarOuAtualizarGrafico(`chart-${sufixo}-cmb`,      labels, cmb,      '#00e57a', 'line');
  criarOuAtualizarGrafico(`chart-${sufixo}-gter`,     labels, gter,     cor, 'bar');

  const n = regs.length;
  const avg = arr=>arr.filter(v=>v!=null).length? (arr.filter(v=>v!=null).reduce((a,b)=>a+b,0)/arr.filter(v=>v!=null).length).toFixed(2) : '-';
  document.getElementById(`info-chart-${sufixo}`).textContent=
    `${n} REGISTROS | ClinGter med: ${avg(clingter)} MW | CMO med: ${avg(cmo)} R$/MWh`;
}

// ── Coluna PDPW sugerido ────────────────────────────────────
...
function trocarTab(id,btn){
  tabAtual=id;
  ['gna1','gna2','ger','term'].forEach(t=>{document.getElementById('tab-'+t).style.display=(t===id)?'block':'none';});
...
function aplicarPeriodo(){
  ...
  // Graficos
  renderGraficos('GNA I', registros);
  renderGraficos('GNA II', registros);
  // Tabela geral
  aplicarFiltros();
...
async function fetchDados(silencioso=false){
  ...
  try{
    ...
    document.getElementById('date-from').value=hoje.toISOString().split('T')[0];
    aplicarPeriodo();
  }catch(err){
    document.getElementById('loading').classList.add('hidden');
...
function renderTabelaGer(registros,registrosTerm){
  ...
  if(!registros.length){
    ...
    tbody.innerHTML='<tr><td><div class="empty"><div class="icon">&#128235;</div>Nenhum registro.</div></td></tr>';
    return;
  }
